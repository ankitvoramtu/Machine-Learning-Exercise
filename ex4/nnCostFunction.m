...
% record the labels as vectors
yVec = repmat([1:num_labels], m, 1) == repmat(y, 1, num_labels);

cost = - yVec .* log(h) - (1 - yVec) .* log(1 - h);

% do not regularize the first column of Theta
Theta1NoBias = Theta1(:, 2:end);
Theta2NoBias = Theta2(:, 2:end);

# sum by rows(all labels for one example), then by columns(all examples)
J = 1/m * sum(sum(cost, dim=2)) + ...
lambda/(2*m) * (sum(sum(Theta1NoBias .^2, dim = 2)) + ...
sum(sum(Theta2NoBias .^2, dim = 2)));
...
for i = 1:m,
  ...
  # for hidden layer
  z2_i = [1; Theta1 * a1_i];
  d2_i = Theta2' * d3_i .* sigmoidGradient(z2_i);
  
  % accumulate the gradient
  delta2 = delta2 + d3_i * a2_i';
  delta1 = delta1 + d2_i(2:end) * a1_i';
end
